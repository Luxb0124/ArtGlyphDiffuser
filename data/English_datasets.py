...
class LetterFewshotDataset(data.Dataset):
    def __init__(self, root=None, few_num=3, nencode=1, test_nums=None, dim=(64, 64),
                 is_fixed_prompt=True, prob_mask_ref=0):
        self.root = root
        self.data = []
        self.dim = dim
    ...
    def __len__(self):
        return len(self.styles) * 26

    def get_prompts(self):
        current_directory = os.path.dirname(__file__)
        prompts_path = os.path.join(current_directory, 'prompts.txt')
        with open(prompts_path, 'r') as f:
            prompts = f.readlines()
        return prompts

    def __getitem__(self, idx):
        style = self.styles[idx // 26]
        ...
        if self.prompts:
            ...
            prompt = "The English letter '%s' in its upper-case appearance." %(dst_char)
...
class LetterFewshotTestDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.prompts is not None:
            ...
            prompt = "The English letter '%s' in its upper-case appearance." % (prompt[-2])
        k_v_dict['txt'] = prompt
...
if __name__ == '__main__':
    root = '../../../../datasets/font_datasets/SEPARATE/Capitals_colorGrad64/train'
    test_nums = 4
